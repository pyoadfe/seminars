...
def get_class_ids(labels):
    ...
    """
    return labels.map(label_class_correspondence)
...
def plot_roc_curves(predictions, labels):
    ...
    for lab in u_labels:
        ...
        plt.plot(fpr, tpr, linewidth=3, label=class_label_correspondence[lab] + ', AUC = ' + str(numpy.round(auc, 4)))
        plt.xlabel("FPR", size=15)
        plt.ylabel('Signal efficiency (TPR)', size=15)
        plt.xticks(size=15)
        plt.yticks(size=15)
        plt.xlim(0., 1)
        plt.ylim(0., 1)
